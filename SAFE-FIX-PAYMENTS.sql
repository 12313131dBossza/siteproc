-- SAFE FIX FOR PAYMENTS TABLE
-- This version handles missing foreign key references gracefully

-- First, check what tables exist
SELECT table_name 
FROM information_schema.tables 
WHERE table_schema = 'public' 
AND table_name IN ('companies', 'projects', 'orders', 'expenses')
ORDER BY table_name;

-- Create payments table WITHOUT foreign key constraints first
-- We'll add them later if the tables exist
...
-- Success message
SELECT 'Payments table created successfully! You can now add payments.' as status;

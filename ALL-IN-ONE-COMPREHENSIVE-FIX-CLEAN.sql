...
begin
  ...
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='payments' and column_name='company_id') then
    ...
    raise notice 'Added payments.company_id';
  end if;
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='payments' and column_name='created_by') then
    ...
    raise notice 'Added payments.created_by';
  end if;
  if not exists (select 1 from information_schema.columns where table_schema='public' and table_name='payments' and column_name='updated_at') then
    ...
    raise notice 'Added payments.updated_at';
  end if;
  ...
  raise notice 'Payments schema normalized';
end $$;
...
begin
  ...
  raise notice 'Dropped existing payments policies';
end $$;
...
begin
  ...
  raise notice 'Contractors schema normalized';
end $$;
...
begin
  ...
  raise notice 'Dropped existing contractors policies';
end $$;
...
begin
  ...
  raise notice 'Clients schema normalized';
end $$;
...
begin
  ...
  raise notice 'Dropped existing clients policies';
end $$;
...
-- DONE! All 5 modules fixed: Projects, Payments, Products, Contractors, Clients

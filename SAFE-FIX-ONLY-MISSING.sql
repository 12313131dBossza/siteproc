-- ============================================================================
-- SAFE DATABASE FIX - ONLY ADD WHAT'S MISSING
-- ============================================================================
-- Based on actual column check, this script only adds missing columns
-- 
...
-- Status constraint (status column exists as text)
do $$
begin
...
-- Status constraint (now that status column exists)
do $$
begin
...
-- ============================================================================
-- 3. CHANGE ORDERS MODULE (Already has company_id, status, approved_at!)
-- ============================================================================
...
-- Status constraint (status column already exists)
do $$
begin
  ...
  raise notice 'Change Orders status constraint added';
exception when others then
  raise notice 'Change Orders status constraint failed: %', sqlerrm;
end $$;

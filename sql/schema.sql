...
-- Profiles (new canonical user profile table; id matches auth.users.id)
-- We retain legacy public.users for now; migrate later.
create table if not exists public.profiles (
  id uuid primary key, -- auth user id
  email text,
  role text, -- mapped to new simplified role hierarchy: viewer|bookkeeper|manager|admin

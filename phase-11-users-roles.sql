...
-- =====================================================
-- This migration implements comprehensive user and role management system
...
ALTER TABLE profiles 
  ...
  ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'active' CHECK (status IN ('active', 'pending', 'inactive', 'suspended')),
...
CREATE TABLE IF NOT EXISTS role_permissions (
  ...
  resource TEXT NOT NULL, -- e.g., 'orders', 'deliveries', 'products', 'users'
...
-- Note: This trigger would be attached to auth.users table if accessible
-- For now, we'll update last_login via API calls
...
CREATE POLICY "user_activity_log_select" ON user_activity_log
  FOR SELECT USING (
    ...
    AND (
      ...
      user_id = auth.uid()
      OR
      -- Admins and owners can see all company activities
      EXISTS (
...
-- Step 12: Update existing profiles RLS policies for role-based access
...
CREATE POLICY "profiles_update" ON profiles
  FOR UPDATE USING (
    ...
    id = auth.uid()
    OR
    -- Admins and owners can update profiles in their company
    (
      company_id IN (
        SELECT company_id FROM profiles WHERE id = auth.uid()
      )

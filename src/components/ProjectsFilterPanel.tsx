...
export function ProjectsFilterPanel({ onFiltersChange }: ProjectsFilterPanelProps) {
  const {
    filters,
    savedFilters,
    applyFilter,
  ...
  const handleQuickFilter = (value: string) => {
    ...
    if (value === 'active') {
      ...
      // Projects using >90% of budget
      applyFilter('budgetUsage', '90');
      onFiltersChange({ budgetUsage: '90' });
    } else if (value === 'over-budget') {
      applyFilter('overBudget', 'true');
      onFiltersChange({ overBudget: 'true' });
    } else {
      onFiltersChange({});
    }
  };
      ...
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {Object.entries(filters).map(([key, value]) => {
            ...
            else if (key === 'minBudget') label = 'Min Budget';
            else if (key === 'maxBudget') label = 'Max Budget';
            else if (key === 'startDate') label = 'From';
            else if (key === 'endDate') label = 'To';
            else if (key === 'budgetUsage') label = 'Budget Usage >';
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <input
                ...
                onChange={(e) => handleApplyFilter('minBudget', e.target.value)}
                placeholder="Min"
              ...
              <input
                ...
                onChange={(e) => handleApplyFilter('maxBudget', e.target.value)}
                placeholder="Max"
                className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
          ...
          <div>
            ...
            <input
              type="number"
              value={filters.budgetUsage || ''}
              onChange={(e) => handleApplyFilter('budgetUsage', e.target.value)}
              placeholder="Show projects using more than..."
              min="0"
              max="100"
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
            <p className="text-xs text-gray-500 mt-1">
              Example: Enter 90 to show projects using &gt;90% of budget
            </p>
          </div>
          ...
          <div>
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={filters.overBudget === 'true'}
                onChange={(e) => handleApplyFilter('overBudget', e.target.checked ? 'true' : '')}
                className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
              <span className="text-sm text-gray-700">Only show over-budget projects</span>
            </label>
          </div>
        </div>
      </FilterPanel>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Active Projects Over Budget"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  autoFocus
                />
              </div>

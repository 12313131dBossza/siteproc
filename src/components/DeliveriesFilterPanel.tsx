...
interface DeliveriesFilterPanelProps {
  onFiltersChange: (filters: any) => void;
}

export function DeliveriesFilterPanel({ onFiltersChange }: DeliveriesFilterPanelProps) {
  const {
    filters,
    savedFilters,
    applyFilter,
  ...
  const handleQuickFilter = (value: string) => {
    ...
    if (value === 'pending' || value === 'partial' || value === 'delivered') {
      applyFilter('status', value);
      onFiltersChange({ status: value });
    } else if (value === 'today') {
      const today = new Date().toISOString().split('T')[0];
      applyFilter('startDate', today);
      applyFilter('endDate', today);
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {Object.entries(filters).map(([key, value]) => {
            ...
            if (key === 'status') {
              label = 'Status';
              if (value === 'partial') displayValue = 'In Transit';
            }
      ...
      >
        <div className="space-y-4">
          ...
          {/* Proof of Delivery Toggle */}
          <div>
            <label className="flex items-center gap-2">
              <input
                type="checkbox"
                checked={filters.hasProof === 'true'}
                onChange={(e) => handleApplyFilter('hasProof', e.target.checked ? 'true' : '')}
                className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
              <span className="text-sm text-gray-700">Only show deliveries with POD</span>
            </label>
          </div>
        </div>
      </FilterPanel>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Today's Deliveries"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  autoFocus
                />
              </div>

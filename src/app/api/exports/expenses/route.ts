...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await sb.from('expenses').select('spent_at,amount,memo,supplier_id,cost_code_id').eq('company_id', companyId).order('spent_at',{ascending:false}).limit(1000) as any
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  const csv = expensesCsv(((data||[]) as any[]).map(r=> ({ ...r })))

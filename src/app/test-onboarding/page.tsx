...
export default function TestOnboarding() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    setCreating(true)
    try {
      const res = await fetch('/api/onboarding/create-temp', {
        method: 'POST',
  ...
  async function handleJoin(e: React.FormEvent) {
    ...
    setJoining(true)
    try {
      const res = await fetch('/api/onboarding/join-temp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      else setErrorJoin(data.details || code || 'Failed to join company')
    } catch (err: any) {
  ...
  return (
    <div className="mx-auto max-w-lg py-16 space-y-8">
      <h1 className="text-2xl font-semibold">Test Onboarding (Temporary)</h1>
      <p className="text-sm text-gray-400">Using hardcoded service key for testing</p>
        ...
        <form onSubmit={handleCreate} className="flex flex-col gap-2">
          <div className="flex gap-2">
            <input 
              ...
              className="flex-1 rounded px-3 py-2 bg-neutral-800" 
            />
            <button 
              disabled={creating} 
      ...
      <section className="space-y-3">
        ...
        <form onSubmit={handleJoin} className="flex flex-col gap-2">
          <div className="flex gap-2">
            ...
            <button 
              disabled={joining} 
              className="bg-green-600 px-4 py-2 rounded text-sm disabled:opacity-50"
            >
      ...
      <section className="space-y-3 border-t pt-4">
        ...
        <div className="text-xs space-y-1">
          <a href="/api/_debug/detailed" target="_blank" className="block text-blue-400 hover:underline">
            Check Detailed Environment Variables
          </a>
          <a href="/api/_debug/envcheck" target="_blank" className="block text-blue-400 hover:underline">

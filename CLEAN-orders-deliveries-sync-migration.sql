-- Orders × Deliveries Sync Migration (Clean Slate - Drop and Recreate)
-- Fix column types and set up sync properly

-- Step 1: Drop existing columns with wrong types
ALTER TABLE purchase_orders 
DROP COLUMN IF EXISTS delivery_progress CASCADE;

ALTER TABLE purchase_orders 
DROP COLUMN IF EXISTS ordered_qty CASCADE;

ALTER TABLE purchase_orders 
DROP COLUMN IF EXISTS delivered_qty CASCADE;

ALTER TABLE purchase_orders 
DROP COLUMN IF EXISTS remaining_qty CASCADE;

ALTER TABLE purchase_orders 
DROP COLUMN IF EXISTS delivered_value CASCADE;

-- Step 2: Add delivery_progress field with correct TEXT type
ALTER TABLE purchase_orders 
ADD COLUMN delivery_progress TEXT DEFAULT 'not_started' 
CHECK (delivery_progress IN ('not_started', 'partially_delivered', 'completed'));

-- Step 3: Add tracking fields for quantities and values
ALTER TABLE purchase_orders 
ADD COLUMN ordered_qty DECIMAL(10,2) DEFAULT 0;

ALTER TABLE purchase_orders 
ADD COLUMN delivered_qty DECIMAL(10,2) DEFAULT 0;

ALTER TABLE purchase_orders 
ADD COLUMN remaining_qty DECIMAL(10,2) DEFAULT 0;

ALTER TABLE purchase_orders 
ADD COLUMN delivered_value DECIMAL(10,2) DEFAULT 0;

-- Step 4: Convert deliveries.order_id from TEXT to UUID
DO $$
DECLARE
    v_data_type TEXT;
BEGIN
...
-- Step 5: Add foreign key constraint from deliveries to purchase_orders
ALTER TABLE deliveries 
...
-- Step 7: Create function to calculate delivery progress (using UUID)
...
-- Step 8: Create trigger to auto-update on delivery status change
...
-- Step 9: Create trigger for delivery_items changes
...
-- Step 10: Initialize ordered_qty from existing quantity field
UPDATE purchase_orders
SET ordered_qty = COALESCE(quantity, 0);

-- Step 11: Recalculate all existing orders' delivery progress
DO $$
DECLARE
    order_record RECORD;
    v_count INT := 0;
